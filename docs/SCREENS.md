...
- **Data:** `GET /api/consent/status?purpose=health_biometric_processing` → `{granted, currentVersion, staleVersion}`; `POST /api/consent` `{purpose:'health_biometric_processing', dataCategories:[…]}`; `POST /api/consent/withdraw` `{purpose}` (§ 10). `dataCategories` canonical identifiers — **and this doc said five where the client sends eight, which is now corrected.** `grantConsent` sends `CONSENT_DATA_CATEGORIES` (`consentApi.ts:45-48`), the **union across every wearable lane**: the five Health Connect ones `['heart_rate','hrv','sleep','resting_heart_rate','historical_access_182d']` **plus the three Garmin-only special-category ones** `['spo2','respiratory_rate','body_battery']`. So every `ConsentRecord` on file records eight. The three Garmin ones are disclosed but **not read** at the current version — `GARMIN_CONSENT_MIN_VERSION` sits above `CURRENT_CONSENT_VERSION`, `garminSpecialCategoryAllowed()` returns false, and `garminIngest` drops them. Health Connect never requests them either; they are absent from `READ_RECORD_TYPES`. **Open point (raised by `compliance-auditor`, not yet resolved):** recording consent to three categories the granted version does not authorise is over-recording. It fails safe — the gate under-processes — but the cleaner design records only what the granted version authorises. A written internal rule contradicted by the shipped code is the worst posture in a supervisory inquiry, which is why this line now states what the code does rather than what was intended. Scope-minimized to the real Health Connect request set (permissions.ts, PR #152 T3: SpO2/respiratory/background dropped, zero readers); must mirror the on-screen data list exactly. `CURRENT_CONSENT_VERSION = 1` is the cross-package staleness contract.
