...
- **Layout:** header (name/avatar/email) → Integrations (Spotify · YouTube Music · Wearable · Health data — consistent status + action) → Health-data "Vault" panel (sync, permissions, what we read/why) → Log out → Delete account → **preferences last** ("Living background" on/off; "How adventurous" slider).
- **Ordering is normative, and the reason is reachability.** The GDPR Art. 15/17/20 controls must sit **above** the preferences, not below them. The canvas board originally ran ~870px of content in a ~682px port with the data controls last, so "Log out", "Export my data" and "Health data" rendered in **no frame at all** — present in the markup, absent from every capture. A rights control that is only reachable by scrolling past a discretionary preference is the wrong hierarchy for a screen whose stated tone is "trust-first".
