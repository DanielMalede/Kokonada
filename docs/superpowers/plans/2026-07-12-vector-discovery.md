...
const catalog = new Map();
jest.mock('../app/repositories/trackCatalogRepo', () => ({ getMany: async (keys) => new Map(keys.filter(k => catalog.has(k)).map(k => [k, catalog.get(k)])) }));
...
describe('discovery integration (fake index)', () => {
  ...
  it('end-to-end: aiParams → vector match → hydrated, non-familiar, isDiscovery candidates', async () => {
    ...
    catalog.set('r1', { recordingKey: 'r1', canonicalKey: 'new1', uri: 'spotify:track:r1', title: 'Fresh', artist: 'New', genres: ['indie'] });
    fake.store.set('r2', { vector: buildVector({ bpm: 100, energy: 0.4, valence: 0.5 }, ['indie']), canonicalKey: 'owned' });
    catalog.set('r2', { recordingKey: 'r2', canonicalKey: 'owned', uri: 'spotify:track:r2', title: 'Have it', artist: 'Known', genres: ['indie'] });
