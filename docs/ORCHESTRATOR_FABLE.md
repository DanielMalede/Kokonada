...
> 🧭 **Invoke the agents + max reasoning:** the six named sub-agents (all Opus) — `architect` (read-only planning), `designer` (read-only design authority + SHIP/REVISE design-review), `developer` (builds under strict TDD), `resilience-auditor` (read-only stress/boundary QA), `compliance-auditor` (read-only external-API/store/branding gate), `integrator` (read-only conflict supervisor — partitions parallel work by disjoint file footprint + predicts merge conflicts). Per task pair a `developer` with the reviewers it needs = one squad; **UI screens also get `designer`**; anything touching a third-party API/store/brand also gets `compliance-auditor`. **Before running decoupled squads in parallel, the `integrator` partitions them by disjoint file footprint — overlapping tasks run sequentially.** For maximum reasoning, put the keyword **`ultrathink`** in the prompt and set the session's highest reasoning level. (No literal "Ultracode/xhigh" toggle exists — model=Opus + `ultrathink` + highest UI level is the real equivalent.)
...
- **Integrator (conflict supervisor)** — read-only (+ git). **Before parallel dispatch**, computes each task's file/module footprint (via the dependency graph) and only parallelizes DISJOINT tasks — overlapping ones go sequential. **Before merges**, sets a dependency-correct merge order and predicts conflicts with `git merge-tree` without mutating anything, then hands real conflicts to the `developer` to resolve test-first. Never edits, never merges. Conflict-freedom is only guaranteed for work routed through the orchestrator.
