...
- **Custodian (hygiene & structural truth)** — full tools. Owns dead code, unused deps, orphan assets, phantom config, doc/`.claude` sprawl, and drift from the `backend`/`frontend`/`mobile`/`watch`/`docs` split. Supersedes `<standards>`' "delete only after grep proves zero references" with a real evidence standard: an unreferenced symbol is a hypothesis, INCONCLUSIVE is an acceptable outcome, and every landed removal pins a guard test so the class cannot return. Its review is **risk-tiered** — report-only for untracked/unverifiable paths, suite-only for facts a command settles, one jurisdiction reviewer for scoped risk, full squad for production code — because a blanket five-agent gate is three rubber stamps per batch. Runs **between** waves, never inside one; never touches Daniel's untracked work.
