...
| Garmin **Training Readiness** (special category) | `MedicalProfile.dailyReadiness` | Recovery axis of the derived physiological state (session targeting); surfaced on Pulse | **GDPR Art.9 special category.** Field-level encrypted per ADR 0005. **Not collected today** — no normalizer emits this metric, so nothing writes the field. It is disclosed in the consent screen as Garmin-sourced (`daily_readiness`) and **hard-gated at ingest**: both ingest lanes DROP it unless the user's latest *granted* consent version is >= `GARMIN_CONSENT_MIN_VERSION`, so the lane cannot open without a fresh, informed re-consent. <!-- verified: backend/app/services/wearable/specialCategoryMetrics.js; gate backend/app/services/wearable/garminIngest.js + healthStore.js; guard backend/tests/garminConsentVersionGate.test.js -->
