...
| Health & fitness (heart rate, HRV, sleep, resting heart rate) | `BiometricLog`, `VitalSample`, `MedicalProfile`, `MorningState` | Core feature: biometric-adaptive playlist generation | Field-level encrypted per ADR 0005; ingested from Garmin / Health Connect / Apple HealthKit / Suunto / sideloaded watch. **SpO₂ and respiratory rate are NOT collected** (scope removed — no reader). **Never** sent to any LLM/AI; raw values never leave the server boundary except in the subject's own authenticated export. |

> **Reading the `VitalSample` schema — a deliberate superset, not a collection scope.** The
> `VitalSample.metric` enum lists `spO2`, `respirationRate`, `bodyBattery` and `stressLevel`
> alongside the metrics actually collected. **A schema is not data collection:** those four have
> **no writer** — no shipped client sends them, the Health Connect scopes for them were removed, and
> the normalizers that could emit them are gated OFF behind consent-v2 capability flags. The enum is
> written once, consent-v2-ready, so that widening collection later is a consent + flag decision
> rather than a schema migration. The "**not collected**" declaration above therefore stays exactly
> true today, and `backend/tests/wave4.retentionDocs.test.js` fails the build if a writer for a
> not-collected metric ever appears while this declaration stands. <!-- verified: backend/app/models/VitalSample.js (enum + the same note); writer allowlist backend/app/services/wearable/metricStore.js VITAL_METRICS_PERSISTED -->
...
| Per-metric physiological samples (`VitalSample`) | **90 days** (default; `VITAL_SAMPLE_RETENTION_DAYS`) | MongoDB TTL index on `recordedAt`. Field-level encrypted per ADR 0005; erased with the account and by per-wearable disconnect, and included in the subject's export, on the same rails as `BiometricLog`. |
| Nightly consolidated readiness/sleep-debt/cosinor/CUSUM history (`MorningState`) | **90 days** (default; `MORNING_STATE_RETENTION_DAYS`) | MongoDB TTL index on `date`. Field-level encrypted per ADR 0005; erased with the account and (all-or-nothing, alongside `MedicalProfile`) by per-wearable disconnect, and included in the subject's export. |
| Learned context-bucket rewards (`RewardEvent`) | **365 days of INACTIVITY** (default; `REWARD_EVENT_RETENTION_DAYS`) | MongoDB TTL index on `updatedAt` — the window is a staleness eviction, not a deletion clock: every play in a context pushes that bucket's expiry out, so what expires is only learning about a context the person has left. Not encrypted and deliberately so: the row holds outcome statistics (`rewardSum`, `count`) addressed by COARSE coordinates (one of six state domains × one of three target bands × one of four six-hour bins) — no vital, no state label, and no track identity of any provider (ADR 0012 §1). Erased with the account and included in the subject's export; NOT erased by per-wearable disconnect, because a bucket's evidence is half behavioural and its state coordinate resolves without a wearable at all. |
| Learned scoring overlay (`PersonalWeights`) | **365 days of INACTIVITY** (default; `PERSONAL_WEIGHTS_RETENTION_DAYS`) | MongoDB TTL index on `updatedAt` — a staleness eviction on the same footing as `RewardEvent`: every observation pushes the expiry out, and what expires is an overlay nobody has reinforced for a year. Expiry is not a loss here — a missing row means COLD START, i.e. the deployment-default scoring weights, which is exactly where the §M.15 shrink-to-global was taking it anyway. Not encrypted and deliberately so: the row holds four bounded music-RANKING coefficients (how much this listener's ranking leans on their own affinity vs. biosonic fit vs. genre vs. proven rotation) — no vital, no state label, no track identity of any provider, and no way to invert one out of a ±0.4 residual shared across every play. Erased with the account and included in the subject's export; NOT erased by per-wearable disconnect, for the same reason `RewardEvent` is not — half the evidence behind it is behavioural (skip / complete / save), which no wearable ever touched. |
...
- **Security practices:**
  ...
  - **Users can request data deletion:** yes — in-app **Delete account** (`DELETE /api/auth/account`) performs a server-first hard delete across every user-owned collection (BiometricLog, VitalSample, MedicalProfile, MorningState, RewardEvent, PersonalWeights, MusicProfile, PlaylistSession, ServeEvent, Identity, RefreshToken, UnclassifiedTrack, ConsentRecord) plus user-scoped Redis state, removes the User doc last, disconnects live sockets, and revokes the session JWT; `scripts/gdpr-delete.js` mirrors the same cascade. A deletion URL will be provided for the Play listing. <!-- verified: backend/app/controllers/authController.js:220-245; backend/app/services/privacy/erasure.js:37-68; membership guarded against drift by backend/tests/wave4.retentionDocs.test.js -->
