...
- **Security practices:**
  ...
  - **Users can request data deletion:** yes — in-app **Delete account** (`DELETE /api/auth/account`) performs a server-first hard delete across every user-owned collection (BiometricLog, VitalSample, MedicalProfile, MorningState, MusicProfile, PlaylistSession, ServeEvent, Identity, RefreshToken, UnclassifiedTrack, ConsentRecord) plus user-scoped Redis state, removes the User doc last, disconnects live sockets, and revokes the session JWT; `scripts/gdpr-delete.js` mirrors the same cascade. A deletion URL will be provided for the Play listing. <!-- verified: backend/app/controllers/authController.js:220-245; backend/app/services/privacy/erasure.js:35-57; membership guarded against drift by backend/tests/wave4.retentionDocs.test.js -->
