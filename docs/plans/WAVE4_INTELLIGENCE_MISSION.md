...
- **M.4 Shrinkage (empirical Bayes):** `μ̂=(n·x̄+k·μ₀)/(n+k)`; k: RHR 20, HRV 15, hour-bins 10 (toward the user's own overall, which shrinks toward population); `confidence=n/(n+k)`. Replaces MIN_SAMPLES cliffs everywhere.
